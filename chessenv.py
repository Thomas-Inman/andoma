...
class chessEnv:
    ...
    def reset(self) -> Union[chess.Board, numpy.ndarray]:
        self.board.reset()
        return get_bitboard(self.board)
    ...
    def step(self, move: chess.Move) -> Union[chess.Board, bool]:
        ...
        if self.board.status()!=chess.Status.VALID:
            print("\n\n\nInvalid\n\n\n")
        return get_bitboard(self.board), eval, (self.board.is_checkmate() or self.board.is_stalemate() or self.board.is_insufficient_material() or self.board.is_fivefold_repetition() or self.board.status()!=chess.Status.VALID) , self.board.status() == chess.Status.VALID
    
    def get_board(self) -> chess.Board:
        return self.board
    
    def next_move(self, depth: int, board: chess.Board, training=False, agent=None, epsilon=0, episode=0):
        return movegeneration.minimax_root_with_value(depth, board, True, agent, epsilon, episode)


def encode_move(move: chess.Move, index_only:bool, white_to_move: bool) -> numpy.ndarray:
...
def decode_move(array: numpy.ndarray, white_to_move: bool) -> chess.Move:
